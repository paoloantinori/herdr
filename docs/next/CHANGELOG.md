...
- Claude Code session restore now replays the `CLAUDE_CONFIG_DIR` reported by the session hook, so sessions started from a non-default config home resume into the right home instead of failing to find the conversation; re-run `herdr integration install claude` to pick up the updated hook.
