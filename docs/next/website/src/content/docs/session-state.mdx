...
Claude Code integration version 8 and newer also report the `CLAUDE_CONFIG_DIR` the session was started with when it points at a non-default config home (macOS and Linux panes; the Windows hook does not report it yet), and restore replays it as part of the resume command so the session resolves in the right home. Names outside each integration's reported-env allowlist are dropped instead of persisted.
