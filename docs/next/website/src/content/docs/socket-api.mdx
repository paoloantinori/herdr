...
Session-only official integrations report native session references with `pane.report_agent_session`. State-reporting integrations can still include native session references in `pane.report_agent`. State-independent session reports do not affect waits, notifications, or rollups. `pane.report_agent_session` is also the only report path that accepts the optional `env` object of integration-reported environment variables; `pane.report_agent` does not.
