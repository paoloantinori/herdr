...
herdr agent restart <name> [--timeout MS] [--cold]
...
`agent restart` stops a named agent and relaunches the same kind in the same pane, resuming the session the agent reported when one exists, with its reported environment replayed through an env prefix. By default a working agent is waited out first, because interrupting mid-turn loses work; `--cold` interrupts immediately with C-c for stuck agents. Claude and Codex are exited with a typed `/exit`; other kinds are exited with C-c. The name is released while the pane returns to its shell and retaken by the relaunch, so a concurrent start taking the name fails the restart with `agent_name_taken`. The default timeout is 60000 milliseconds; explicit values must be greater than 3000 and no more than 300000.
