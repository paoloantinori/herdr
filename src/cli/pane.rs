...
fn pane_report_agent_session(args: &[String]) -> std::io::Result<i32> {
    const USAGE: &str = "usage: herdr pane report-agent-session <pane_id> --source ID --agent LABEL [--seq N] [--agent-session-id ID] [--agent-session-path PATH] [--session-start-source SOURCE] [--env NAME=VALUE]";
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "--env" => {
                ...
                let (name, value) = match super::parse_env_assignment(value) {
                    Ok(env) => env,
                    Err(message) => {
                        eprintln!("{message}");
                        return Ok(2);
...
fn print_pane_help() {
    ...
    eprintln!("  herdr pane report-agent-session <pane_id> --source ID --agent LABEL [--seq N] [--agent-session-id ID] [--agent-session-path PATH] [--session-start-source SOURCE] [--env NAME=VALUE]");
